...
% Author: Orbital Mechanics with Matlab
% Updated by Shamil Biktmirov on July 7, 2020
...
%             (0 <= true anomaly <= 2 pi)
%  oeosc(7) = osculating mean anomaly (radians)
%             (0 <= true anomaly <= 2 pi)
%  oeosc(8) = osculating argument of latitude (radians)
%             (0 <= true anomaly <= 2 pi)
%  oeosc(9) = osculating tuue mean anomaly (radians)
%             (0 <= true anomaly <= 2 pi)
...
%              (0 <= true anomaly <= 2 pi)
%  oemean(7) = mean mean anomaly (radians)
%              (0 <= true anomaly <= 2 pi)
%  oemean(8) = mean argument of latitude (radians)
%              (0 <= true anomaly <= 2 pi)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

req = Consts.rEarthEquatorial;
j2 = Consts.earthJ2;
...
oemean(2) = em;
oemean(3) = im;
oemean(4) = mod(ranm, 2*pi);
oemean(5) = mod(apm, 2*pi);
oemean(7) = mod(mam, 2*pi);

[~,ta] = newtonm (em,oemean(7));

oemean(6) = ta;
oemean(8) = mod(oemean(5) + oemean(6),2*pi);
oemean(9) = mod(oemean(5) + oemean(7),2*pi);