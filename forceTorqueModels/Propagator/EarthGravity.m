...
[P, scaleFactor] = loc_gravLegendre(phi, degree);
...
for n = 2:degree 
    ...
    for m = 0:n
        j = m+1;
        dUdrSumM      = dUdrSumM + reshape(P(k,j),size(r))*(C(n,j)*cos(m*lambda) + S(n,j)*sin(m*lambda));
        dUdphiSumM    = dUdphiSumM + ( (reshape(P(k,j+1),size(r))*reshape(scaleFactor(k,j),size(r))) - tan(phi)*m*reshape(P(k,j),size(r)))*(C(n,j)*cos(m*lambda) + S(n,j)*sin(m*lambda));
        dUdlambdaSumM = dUdlambdaSumM + m*reshape(P(k,j), size(r))*(S(n,j)*cos(m*lambda) - C(n,j)*sin(m*lambda));
    end
...
a1 = (1/r)*dUdr - (z*dUdphi/(r*r*sqrt(x*x+y*y)));
a2 = dUdlambda/(x*x + y*y);

%Perturbing acceleration
gx = a1*x - a2*y;
gy = a1*y + a2*x; 
gz = dUdr*z/r + sqrt(x*x + y*y)*dUdphi/(r*r);

atPole = abs(atan2(z,sqrt(x^2 + y^2)))==pi/2;
if any(atPole)
    gx(atPole) = 0;
    gy(atPole) = 0;
    gz(atPole) = (1/r(atPole))*dUdr(atPole)*z(atPole);
end
